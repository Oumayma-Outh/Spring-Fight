    ...
    public void render(float delta) {
        ...
        if (delayStarted) {
            ...
            if (delayTimer <= 0f) {
                ...
                // decrease the delay timer by delta time
                delayTimer -= delta;
            }
        ...
        game.shapeRenderer.rect(viewport.getWorldWidth() / 2f - PROGRESS_BAR_BACKGROUND_WIDTH / 2f,
                viewport.getWorldHeight() / 2f - PROGRESS_BAR_BACKGROUND_HEIGHT / 2f, PROGRESS_BAR_BACKGROUND_WIDTH,
                PROGRESS_BAR_BACKGROUND_HEIGHT);
        game.shapeRenderer.setColor(GlobalVariables.GOLD);
        game.shapeRenderer.rect(viewport.getWorldWidth() / 2f - PROGRESS_BAR_MAX_WIDTH / 2f, viewport.getWorldHeight() / 2f -
                PROGRESS_BAR_HEIGHT / 2f, game.assets.manager.getProgress() * PROGRESS_BAR_MAX_WIDTH, PROGRESS_BAR_HEIGHT);
